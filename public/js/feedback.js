...
(function($){
    ...
    var settings = $.extend({
			...
			tpl: {
				...
				overview:		'<div id="feedback-overview"><div class="feedback-logo">Feedback</div><div id="feedback-overview-description"><div id="feedback-overview-description-text"><h3>Title</h3><h3>Description</h3></div></div><div id="feedback-overview-screenshot"><h3>Screenshot</h3></div><div class="feedback-buttons"><button id="feedback-submit" class="feedback-submit-btn feedback-btn-blue">Submit</button><button id="feedback-overview-back" class="feedback-back-btn feedback-btn-gray">Back</button></div><div id="feedback-overview-error">Please enter a description.</div><div class="feedback-wizard-close"></div></div>',
		...
		if (supportedBrowser) {
			...
			$(document).on('click', settings.feedbackButton, function(){
				...
				tpl += settings.tpl.highlighter + settings.tpl.overview + '<canvas id="feedback-canvas"></canvas><div id="feedback-helpers"></div><input id="feedback-note" name="feedback-note" type="hidden"><input id="feedback-title" name="feedback-title" type="hidden"></div>';
					...
					html2canvas($('body'), {
						onrendered: function(canvas) {
							...
							if(settings.showDescriptionModal) {
								...
								$('#feedback-overview-description-text>input').remove();
								$('#feedback-overview-screenshot>img').remove();
								$('<input type="text" id="feedback-overview-title">' + $('#feedback-title').val() + '</textarea>').insertAfter('#feedback-overview-description-text h3:eq(0)');
				...
				$(document).on('keyup', '#feedback-title-tmp,#feedback-overview-title', function(e) {
					var tx;
					if (e.target.id === 'feedback-title-tmp')
						tx = $('#feedback-title-tmp').val();
					else {
						tx = $('#feedback-overview-title').val();
						$('#feedback-title-tmp').val(tx);
					}

					$('#feedback-title').val(tx);
				});
				...
				$(document).on('click', '#feedback-submit', function() {
					...
					if ($('#feedback-note').val().length > 0) {
                        ...
                        data = $.extend(data, settings.extraPostParams);
						$.ajax({
